...
async function startBot() {
  ...
  if (data.strategy === "grid") {
    ...
    data.grid_upper_price = formData.get("grid_upper_price")
      ? parseFloat(formData.get("grid_upper_price"))
      : null;
    data.grid_lower_price = formData.get("grid_lower_price")
      ? parseFloat(formData.get("grid_lower_price"))
      : null;
    ...
    if (data.market_type === "perp") {
      ...
      data.target_position = targetPositionRaw
        ? parseFloat(targetPositionRaw)
        : 0.0;
    ...
    if (data.market_type === "spot") {
      ...
      data.rebalance_threshold = parseFloat(
        formData.get("rebalance_threshold")
      );
    } else {
      data.max_orders = parseInt(
        document.getElementById("max_orders_perp").value
      );
      ...
      data.stop_loss = formData.get("stop_loss")
        ? parseFloat(formData.get("stop_loss"))
        : null;
      data.take_profit = formData.get("take_profit")
        ? parseFloat(formData.get("take_profit"))
        : null;
...
function updateStatsDisplay(stats) {
  ...
  updateStatValue(
    "statPrice",
    stats.current_price ? stats.current_price.toFixed(4) : "--"
  );
  updateStatValue(
    "statTotalBalance",
    stats.total_balance_usd ? `$${stats.total_balance_usd.toFixed(2)}` : "--"
  );
  ...
  if (stats.cumulative_pnl !== undefined && stats.cumulative_pnl !== null) {
    updateStatValue(
      "statCumulativePnL",
      formatPnL(stats.cumulative_pnl),
  ...
  updateStatValue(
    "statBuySell",
    `${stats.buy_trades || 0} / ${stats.sell_trades || 0}`
  );

  // 更新成交額和手續費
  updateStatValue(
    "statVolumeUSDC",
    stats.total_volume_usdc ? `$${stats.total_volume_usdc.toFixed(2)}` : "--"
  );
  updateStatValue(
    "statFees",
    stats.total_fees
      ? `${stats.total_fees.toFixed(4)} ${stats.quote_asset || ""}`
      : "--"
  );
  ...
  // 更新成交量
  const makerTotal =
    (stats.maker_buy_volume || 0) + (stats.maker_sell_volume || 0);
  const takerTotal =
    (stats.taker_buy_volume || 0) + (stats.taker_sell_volume || 0);
  ...
  if (stats.grid_profit !== undefined || stats.grid_count !== undefined) {
    ...
    // 更新價格區間
    if (
      stats.grid_upper_price !== undefined &&
      stats.grid_lower_price !== undefined
    ) {
      const priceRange = `${stats.grid_lower_price.toFixed(
        4
      )} ~ ${stats.grid_upper_price.toFixed(4)}`;
    ...
    let filledText = "--";
    if (
      stats.grid_buy_filled !== undefined &&
      stats.grid_sell_filled !== undefined
    ) {
      // 現貨網格
      filledText = `${stats.grid_buy_filled || 0} / ${
        stats.grid_sell_filled || 0
      }`;
    } else if (
      stats.grid_long_filled !== undefined &&
      stats.grid_short_filled !== undefined
    ) {
      // 永續合約網格
      filledText = `${stats.grid_long_filled || 0} / ${
        stats.grid_short_filled || 0
      }`;
    }
